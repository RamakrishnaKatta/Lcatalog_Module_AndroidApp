...
public class Parser3ds {
    ...
    Parser3ds(InputStream file, final Context context) {
        ...
        for (int i = 0; i < scales.size(); i++) {
            if (scales.get(i) > scaleFactor) {
                scaleFactor = scales.get(i);
                initialScaleFactor = scales.get(i);
            }
        }
        objReady = true;
    }
    ...
    private void parseFaceMaterial() throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            getByte();
            getByte(); //Just skipping these chunks
        }
    }
    ...
    void draw(float[][] mvp, int mMVPMatrixHandle,
        ...
        for (Object3ds obj : models) {
            ...
            if (obj.getVertices() != null) {
                ...
                GLES20.glVertexAttribPointer(mPositionHandle, mPositionDataSize, GLES20.GL_FLOAT, false, 0, obj.getPositionBuffer());
                GLES20.glEnableVertexAttribArray(mPositionHandle);

                // Pass in the normal information
                obj.resetNormalBuffer();
                GLES20.glVertexAttribPointer(mNormalHandle, mPositionDataSize, GLES20.GL_FLOAT, false, 0, obj.getNormalBuffer());
                ...
                GLES20.glVertexAttribPointer(mTextureCoordinateHandle, mTextureCoordinateDataSize, GLES20.GL_FLOAT, false, 0, obj.getTextureBuffer());
